...
static void	ft_get_delta_dist(t_cub *cub)
{
	if (cub->ray.ratdir_x == 0)
		...
		cub->ray.delta_dist_x = fabs(1 / cub->ray.ratdir_x);
	if (cub->ray.ratdir_y == 0)
		cub->ray.delta_dist_y = 1e30;
	else
		cub->ray.delta_dist_y = fabs(1 / cub->ray.ratdir_y);
}
...
void	ft_init_player_view(int x, t_cub *cub)
{
	...
	cub->ray.ratdir_x = cub->ray.dir_x + cub->ray.plane_x * camera_x;
	cub->ray.ratdir_y = cub->ray.dir_y + cub->ray.plane_y * camera_x;
